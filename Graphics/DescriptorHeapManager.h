...
private:
    ...
    public:
        DescriptorHeapAllocator(D3D12_DESCRIPTOR_HEAP_TYPE Type)
            : m_type(Type), m_pCurrentHeap(nullptr)
        {
        }
        ...
        D3D12_CPU_DESCRIPTOR_HANDLE Allocate(uint32_t Count)
        {
            if (m_pCurrentHeap == nullptr || m_remainingFreeHandles < Count)
            {
                ...
                {
                    m_descriptorSize = g_pDevice->GetDescriptorHandleIncrementSize(m_type);
                }
            }
    ...
    protected:
        ID3D12DescriptorHeap* RequestNewHeap(D3D12_DESCRIPTOR_HEAP_TYPE Type)
        {
            ...
            ASSERT_HR(g_pDevice->CreateDescriptorHeap(&Desc, IID_PPV_ARGS(&pHeap)));
