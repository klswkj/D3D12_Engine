...
void SSAOPass::ExecutePass()
{
	...
	if (!m_bEnable)
	{
		...
		graphicsContext.PIXBeginEvent(L"4_SSAOPass", 0);
		graphicsContext.PIXBeginEvent(L"SSAO_Linearize", 0);

		graphicsContext.TransitionResource(bufferManager::g_SSAOFullScreen, D3D12_RESOURCE_STATE_RENDER_TARGET);
		graphicsContext.SubmitResourceBarriers(0);
		graphicsContext.ClearColor(bufferManager::g_SSAOFullScreen, 0);
		...
		{
			graphicsContext.PIXEndEvent(0); // End SSAO_Linearize
			graphicsContext.PIXEndEvent(0); // End 4_SSAOPass
		...
		*/
		...
		computeContext.PIXBeginEvent(L"SSAO_ComputeLinearZ", 0);
		computeContext.TransitionResource(LinearDepth,     D3D12_RESOURCE_STATE_UNORDERED_ACCESS);
		...
		computeContext.PIXEndEvent(0); // End SSAO_ComputeLinearZ

		if (m_bDebugDraw)
		{
			computeContext.PIXBeginEvent(L"SSAO_DebugDraw", 0);
	...
	computeContext.TransitionResource(bufferManager::g_SceneDepthBuffer, D3D12_RESOURCE_STATE_NON_PIXEL_SHADER_RESOURCE);
	computeContext.SubmitResourceBarriers(0u);
	computeContext.TransitionResource(bufferManager::g_SSAOFullScreen, D3D12_RESOURCE_STATE_UNORDERED_ACCESS);
	computeContext.SubmitResourceBarriers(0u);
	...
	// CL #0
	{
		computeContext.PIXBeginEvent(L"Decompressing and Downsampling Buffer", 0);
		{
			computeContext.SetConstant(0, *reinterpret_cast<UINT*>(&ZCoefficient), 0u, 0u);
			computeContext.SetDynamicDescriptor(3, 0, bufferManager::g_SceneDepthBuffer.GetDepthSRV(), 0u);
			...
			computeContext.SetDynamicDescriptors(2, 0, 5, DownsizeUAVs, 0);

			computeContext.SetPipelineState(m_DepthPrepareCS_16, 0);
		...
		}
		{
			if (2 < m_HierarchyDepth)
			{
				...
				computeContext.SetConstants(0, *reinterpret_cast<UINT*>(&temp[0]), *reinterpret_cast<UINT*>(&temp[1]), 0u); // (b0)
				...
				computeContext.SetDynamicDescriptors(2U, 0U, 4U, DownsizeAgainUAVs, 0u);
				...
				computeContext.Dispatch2D((uint64_t)bufferManager::g_DepthTiled4.GetWidth() * NUM_GROUP_THREAD_WIDTH, (uint64_t)bufferManager::g_DepthTiled4.GetHeight() * NUM_GROUP_THREAD_HEIGHT, 8U, 8U, 0u);
			}
		}
	...
	{
		computeContext.PIXBeginEvent(L"Analyzing Depth Range", 0);
		...
		{
			computeContext.SetPipelineState(m_RenderWithWideSamplingCS, 0);
			ComputeAO(computeContext, 0, bufferManager::g_AOMerged4, bufferManager::g_DepthTiled4, FovTangent);
			if (QualityLevel::kLow <= m_eQuality)
			{
				computeContext.SetPipelineState(m_RenderWithDepthArrayCS, 0);
				ComputeAO(computeContext, 0, bufferManager::g_AOHighQuality4, bufferManager::g_DepthDownsize4, FovTangent);
			}
		}
		if (2 < m_HierarchyDepth)
		{
			computeContext.SetPipelineState(m_RenderWithWideSamplingCS, 0);
			ComputeAO(computeContext, 0, bufferManager::g_AOMerged3, bufferManager::g_DepthTiled3, FovTangent);
			if (QualityLevel::kMedium <= m_eQuality)
			{
				computeContext.SetPipelineState(m_RenderWithDepthArrayCS, 0);
				ComputeAO(computeContext, 0, bufferManager::g_AOHighQuality3, bufferManager::g_DepthDownsize3, FovTangent);
			}
		}
		if (1 < m_HierarchyDepth)
		{
			computeContext.SetPipelineState(m_RenderWithWideSamplingCS, 0);
			ComputeAO(computeContext, 0, bufferManager::g_AOMerged2, bufferManager::g_DepthTiled2, FovTangent);
			if (QualityLevel::kHigh <= m_eQuality)
			{
				computeContext.SetPipelineState(m_RenderWithDepthArrayCS, 0);
				ComputeAO(computeContext, 0, bufferManager::g_AOHighQuality2, bufferManager::g_DepthDownsize2, FovTangent);
			}
		}

		{
			computeContext.SetPipelineState(m_RenderWithWideSamplingCS, 0);
			ComputeAO(computeContext, 0, bufferManager::g_AOMerged1, bufferManager::g_DepthTiled1, FovTangent);
			if (QualityLevel::kVeryHigh <= m_eQuality)
			{
				computeContext.SetPipelineState(m_RenderWithDepthArrayCS, 0);
				ComputeAO(computeContext, 0, bufferManager::g_AOHighQuality1, bufferManager::g_DepthDownsize1, FovTangent);
			}
		}
		computeContext.PIXEndEvent(0); // End Analyzing Depth Range
		computeContext.ExecuteCommands(0u, false, true);
	...
	{
		computeContext.PIXBeginEvent(L"Blur and Upsampling", 0);
		...
		if (3 < m_HierarchyDepth)
		{
			BlurAndUpsampling
			(
				computeContext, 0, bufferManager::g_AOSmooth3, bufferManager::g_DepthDownsize3, bufferManager::g_DepthDownsize4,
		...
		if (2 < m_HierarchyDepth)
		{
			BlurAndUpsampling
			(
				computeContext, 0, bufferManager::g_AOSmooth2, bufferManager::g_DepthDownsize2, bufferManager::g_DepthDownsize3,
		...
		if (1 < m_HierarchyDepth)
		{
			BlurAndUpsampling
			(
				computeContext, 0, bufferManager::g_AOSmooth1, bufferManager::g_DepthDownsize1, bufferManager::g_DepthDownsize2,
		...
		BlurAndUpsampling
		(
			computeContext, 0, bufferManager::g_SSAOFullScreen, LinearDepth, bufferManager::g_DepthDownsize1,
			InterleavedAO, (QualityLevel::kVeryHigh <= m_eQuality) ? &bufferManager::g_AOHighQuality1 : nullptr, nullptr
		);

		computeContext.PIXEndEvent(0); // End Blur and Upsampling
	...
	if (m_bDebugDraw)
	{
		...
		computeContext.PIXBeginEvent(L"SSAO_Debug_Draw", 0);

		computeContext.TransitionResource(bufferManager::g_SceneColorBuffer, D3D12_RESOURCE_STATE_UNORDERED_ACCESS);
		computeContext.TransitionResource(bufferManager::g_SSAOFullScreen,   D3D12_RESOURCE_STATE_NON_PIXEL_SHADER_RESOURCE);
		computeContext.SubmitResourceBarriers(0u);

		// NewComputeContext.SetRootSignature(m_MainRootSignature);
		computeContext.SetRootSignature(m_LinearizeDepthSignature, 0u);
		computeContext.SetPipelineState(m_DebugSSAOCS, 0u);
		computeContext.SetDynamicDescriptors(1, 0, 1, &bufferManager::g_SceneColorBuffer.GetUAV(), 0u);
		computeContext.SetDynamicDescriptors(2, 0, 1, &bufferManager::g_SSAOFullScreen.GetSRV(), 0u);
		computeContext.Dispatch2D(bufferManager::g_SSAOFullScreen.GetWidth(), bufferManager::g_SSAOFullScreen.GetHeight(), 0u);

		computeContext.PIXEndEvent(0); // End DebugDraw
		computeContext.PIXEndEvent(0); // End 4_SSAOPass
		computeContext.Finish();
...
)
{
	...
	static __declspec(align(16)) float s_SsaoCB[28];

	::EnterCriticalSection(&m_CS);

	// TODO 0 : ThicknessMultiplier DirtyBit로 바뀌는지 기록하기.
	if (m_FovTangent != TangentHalfFOVHorizontal)
	{
		m_FovTangent = TangentHalfFOVHorizontal;
		...
		s_SsaoCB[0] = InverseRangeFactor / m_SampleThickness[0];
		s_SsaoCB[1] = InverseRangeFactor / m_SampleThickness[1];
		s_SsaoCB[2] = InverseRangeFactor / m_SampleThickness[2];
		s_SsaoCB[3] = InverseRangeFactor / m_SampleThickness[3];
		s_SsaoCB[4] = InverseRangeFactor / m_SampleThickness[4];
		s_SsaoCB[5] = InverseRangeFactor / m_SampleThickness[5];
		s_SsaoCB[6] = InverseRangeFactor / m_SampleThickness[6];
		s_SsaoCB[7] = InverseRangeFactor / m_SampleThickness[7];
		s_SsaoCB[8] = InverseRangeFactor / m_SampleThickness[8];
		s_SsaoCB[9] = InverseRangeFactor / m_SampleThickness[9];
		s_SsaoCB[10] = InverseRangeFactor / m_SampleThickness[10];
		s_SsaoCB[11] = InverseRangeFactor / m_SampleThickness[11];
		...
		// are normalized.
		s_SsaoCB[12] = 4.0f * m_SampleThickness[0];    // Axial
		s_SsaoCB[13] = 4.0f * m_SampleThickness[1];    // Axial
		s_SsaoCB[14] = 4.0f * m_SampleThickness[2];    // Axial
		s_SsaoCB[15] = 4.0f * m_SampleThickness[3];    // Axial
		s_SsaoCB[16] = 4.0f * m_SampleThickness[4];    // Diagonal
		s_SsaoCB[17] = 8.0f * m_SampleThickness[5];    // L-shaped
		s_SsaoCB[18] = 8.0f * m_SampleThickness[6];    // L-shaped
		s_SsaoCB[19] = 8.0f * m_SampleThickness[7];    // L-shaped
		s_SsaoCB[20] = 4.0f * m_SampleThickness[8];    // Diagonal
		s_SsaoCB[21] = 8.0f * m_SampleThickness[9];    // L-shaped
		s_SsaoCB[22] = 8.0f * m_SampleThickness[10];   // L-shaped
		s_SsaoCB[23] = 4.0f * m_SampleThickness[11];   // Diagonal
	...
	// If we aren't using all of the samples, delete their weights before we normalize.
		...
		{
			totalWeight += s_SsaoCB[i];
		}

		for (size_t i = 12; i < 24; ++i)
		{
			s_SsaoCB[i] /= totalWeight;
		}

		s_SsaoCB[24] = 1.0f / BufferWidth;
		s_SsaoCB[25] = 1.0f / BufferHeight;
		s_SsaoCB[26] = 1.0f / -m_RejectionFallOff;
		s_SsaoCB[27] = 1.0f / (1.0f + m_Accentuation);
	}

	::LeaveCriticalSection(&m_CS);

	Context.SetDynamicConstantBufferView(1, sizeof(s_SsaoCB), s_SsaoCB, commandIndex);
	Context.SetDynamicDescriptor(2, 0, Result.GetUAV(), commandIndex);
	Context.SetDynamicDescriptor(3, 0, _DepthBuffer.GetSRV(), commandIndex);

	if (ArrayCount == 1)
	{
		Context.Dispatch2D(BufferWidth, BufferHeight, 16, 16, commandIndex);
	}
	else
	{
		Context.Dispatch3D(BufferWidth, BufferHeight, ArrayCount, 8, 8, 1, commandIndex);
	}
}
...
)
{
	...
	computeContext.SetDynamicDescriptor(2, 0, Result.GetUAV(), commandIndex);
	computeContext.SetDynamicDescriptor(3, 0, LowResolutionDepth.GetSRV(), commandIndex);
	computeContext.SetDynamicDescriptor(3, 1, HighResolutionDepth.GetSRV(), commandIndex);
	...
	if (InterleavedAO != nullptr)
	{
		...
		computeContext.SetDynamicDescriptor(3, 2, InterleavedAO->GetSRV(), commandIndex);
	}
	if (HighQualityAO != nullptr)
	{
		computeContext.TransitionResource(*HighQualityAO, D3D12_RESOURCE_STATE_NON_PIXEL_SHADER_RESOURCE);
		computeContext.SubmitResourceBarriers(commandIndex);
		computeContext.SetDynamicDescriptor(3, 3, HighQualityAO->GetSRV(), commandIndex);
	}
	if (HighResolutionAO != nullptr)
	{
		computeContext.TransitionResource(*HighResolutionAO, D3D12_RESOURCE_STATE_NON_PIXEL_SHADER_RESOURCE);
		computeContext.SubmitResourceBarriers(commandIndex);
		computeContext.SetDynamicDescriptor(3, 4, HighResolutionAO->GetSRV(), commandIndex);
	}
...
void SSAOPass::Execute()
{
	...
	if (!m_bEnable)
	{
		...
		graphicsContext.TransitionResource(SSAOTarget, D3D12_RESOURCE_STATE_RENDER_TARGET);
		graphicsContext.SubmitResourceBarriers(0);
		graphicsContext.ClearColor(SSAOTarget, 0);
...
computeContext.SetConstant(0U, *reinterpret_cast<UINT*>(&ZCoefficient), 0u, 0u);
computeContext.SetDynamicDescriptor(1U, 0U, LinearDepth.GetUAV(), 0u);
computeContext.SetDynamicDescriptor(2U, 0U, MainDepthBuffer.GetDepthSRV(), 0u); // Input : DepthBuffer
computeContext.Dispatch2D(LinearDepth.GetWidth(), LinearDepth.GetHeight(), 16ul, 16ul, 0u);
computeContext.PIXEndEvent(0); // End SSAO_ComputeLinearZ
...
if (m_bDebugDraw)
{
	...
	computeContext.SetRootSignature(m_LinearizeDepthSignature, 0u);
	computeContext.SetPipelineState(m_DebugSSAOCS, 0u);
	computeContext.SetDynamicDescriptors(1U, 0U, 1U, &MainRenderTarget.GetUAV(), 0u); // Output(Will be RenderTarget)
	computeContext.SetDynamicDescriptors(2U, 0U, 1U, &LinearDepth.GetSRV(), 0u);      // Input
	computeContext.Dispatch2D(SSAOTarget.GetWidth(), SSAOTarget.GetHeight(), 0u);
	computeContext.PIXEndEvent(0); // End SSAO_DebugDraw
...
return;
	...
	custom::ComputeContext& computeContext = custom::ComputeContext::Begin(1);
	computeContext.TransitionResource(MainDepthBuffer, D3D12_RESOURCE_STATE_NON_PIXEL_SHADER_RESOURCE);
	computeContext.TransitionResource(SSAOTarget, D3D12_RESOURCE_STATE_UNORDERED_ACCESS);
	...
	// CL #0
	{
		...
		{
			computeContext.SetConstant(0, *reinterpret_cast<UINT*>(&ZCoefficient), 0u, 0u);
			computeContext.SetDynamicDescriptor(3, 0, MainDepthBuffer.GetDepthSRV(), 0u);
			...
			computeContext.Execute(false, 0u);
		}
		{
			if (2 < m_HierarchyDepth)
			{
	...
	// Analyzing Depth Range
	{
		...
		computeContext.PIXEndEvent(0); // End Analyzing Depth Range
		computeContext.Execute(false, 0);
	}// End Analyzing Depth Range
