...
namespace custom
{
	...
	{
	public:
		static CopyContext& Begin(const uint8_t numCommandALs);
		...
		static uint64_t ReadbackTexture2D(GPUResource& readbackBuffer, PixelBuffer& srcBuffer, const UINT readbackBufferFirstSubResourceIndex, const UINT srcBufferFirstSubResourceIndex, const UINT numSubResources);
		...
		void CopyCounter(GPUResource& dest, size_t destOffset, StructuredBuffer& src, const uint8_t commandIndex);
		void ResetCounter(StructuredBuffer& Buf, float value = 0, const uint8_t commandIndex = -1);
	private:
	};
}