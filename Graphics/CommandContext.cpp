...
namespace custom
{
    ...
    uint64_t CommandContext::Flush(bool WaitForCompletion)
    {
		...
		{
			device::g_commandContextManager.WaitForFence(FenceValue);
		}
    ...
    {
        LinearBuffer TempSpace = m_cpuLinearAllocator.Allocate(NumBytes, 512);
	...
	void GraphicsContext::SetConstants(UINT RootIndex, uint32_t size, uint32_t* X, ...)
	{
		va_list Args;
		va_start(Args, X);
		...
		{
			m_CommandList->SetGraphicsRoot32BitConstant(RootIndex, Args[i], i);
		}
	}
}
