...
namespace HashInternal
{
	...
	template <typename T> __forceinline constexpr T AlignArbitrary(T value, uint64_t alignment)
	{
		return (T)((((uint64_t)value + alignment - 1) / alignment) * alignment);
	}
	...
	__forceinline const uint8_t Log2(uint64_t value)
	{
		unsigned long mssb = 0; // most significant set bit
		unsigned long lssb = 0; // least significant set bit
